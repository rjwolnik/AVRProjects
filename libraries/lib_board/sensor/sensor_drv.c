...
//!
//! @brief DVK90CAN1 sensor drivers routines.
//!
//! These routines need FOSC #define of "config.h" file.
//!
...
S8 get_temperature (void)
    ...
    for (i=39; i< 76; i++)        // Starting from +25 to +59°C
        {
        if (therm_value >= thermistor_table[i])
            {
            break;
            }
        }
...
U8 get_luminosity (void)
    ...
    for (i=1; i< 48; i+=2)        // Starting from 0 lux
        {
        if (ldres_value >= luminosity_table[i])
            {
            break;
            }
        }
...
U16 get_vin (void)
    ...
    U32 vin_value;
    U32 temp_value;
    U16 voltage_result, i;
    
    // Channel initialization
    adc_init(INTERNAL_VREF, NO_LEFT_ADJUST, VIN_CHANNEL);
    // To stabilize Aref falling level
    for (i=0; i<((U16)(0x200<<(ADC_PRESCALER-1))); i++) asm("nop");
    // Get ADC value
