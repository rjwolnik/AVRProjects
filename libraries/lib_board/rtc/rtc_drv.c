...
U8  j, k;
    ...
    if (rtc_running == ON)
    {
        asm("cli"); temp = rtc_tics; asm("sei");    //- Get atomic U32 value
        temp += (U32)ms_count;
        while (1)
        {
            asm("cli"); temp = rtc_tics; asm("sei");    //- Get atomic U32 value
            if (rtc_tics != temp) break;
        }
    }
    else    //- No RTC, enabling something almost equivalent but not true.
    {
...
//!
//! @warning RTC_TIMER & RTC_CLOCK must be define in "board.h" and FOSC in
//!          "config.h.
//!
//! @param (none)
//!
