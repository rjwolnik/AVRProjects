...
#       define Uart_rx_in_pu_on()   { UART_0_PORT_DIR &= ~(1<<UART_0_RX_PIN); UART_0_PORT_IN  |=  (1<<UART_0_RX_PIN); }   // RxD in input + pull-up
#       define Uart_rx_in_pu_off()  { UART_0_PORT_DIR &= ~(1<<UART_0_RX_PIN); UART_0_PORT_IN  &= ~(1<<UART_0_RX_PIN); }   // RxD in input no pull-up
#       define Uart_rx_get_data()   ((UART_0_PORT_IN  &   (1<<UART_0_RX_PIN)) >> UART_0_RX_PIN )                          // Read RxD pin
#       define Uart_rx_get_3_data() ((UART_0_PORT_IN  & UART_0_PORT_IN  & UART_0_PORT_IN  & (1<<UART_0_RX_PIN)) >> UART_0_RX_PIN )  // Read 3 times RxD pin
#       define Uart_tx_in_pu_on()   { UART_0_PORT_DIR &= ~(1<<UART_0_TX_PIN); UART_0_PORT_IN  |=  (1<<UART_0_TX_PIN); }   // TxD in input + pull-up
#       define Uart_tx_in_pu_off()  { UART_0_PORT_DIR &= ~(1<<UART_0_TX_PIN); UART_0_PORT_IN  &= ~(1<<UART_0_TX_PIN); }   // TxD in input no pull-up
#       define Uart_set_tx()        { UART_0_PORT_DIR |=  (1<<UART_0_RX_PIN); UART_0_PORT_OUT |=  (1<<UART_0_RX_PIN); }   // TxD in output + set pin
#       define Uart_clear_tx()      { UART_0_PORT_DIR |=  (1<<UART_0_RX_PIN); UART_0_PORT_OUT &= ~(1<<UART_0_RX_PIN); }   // TxD in output + clear pin
...
#       define Uart_rx_in_pu_on()   { UART_1_PORT_DIR &= ~(1<<UART_1_RX_PIN); UART_1_PORT_IN  |=  (1<<UART_1_RX_PIN); }   // RxD in input + pull-up
#       define Uart_rx_in_pu_off()  { UART_1_PORT_DIR &= ~(1<<UART_1_RX_PIN); UART_1_PORT_IN  &= ~(1<<UART_1_RX_PIN); }   // RxD in input no pull-up
#       define Uart_rx_get_data()   ((UART_1_PORT_IN  &   (1<<UART_1_RX_PIN)) >> UART_1_RX_PIN )                          // Read RxD pin
#       define Uart_rx_get_3_data() ((UART_1_PORT_IN  & UART_1_PORT_IN  & UART_1_PORT_IN  & (1<<UART_1_RX_PIN)) >> UART_1_RX_PIN )  // Read 3 times RxD pin
#       define Uart_tx_in_pu_on()   { UART_1_PORT_DIR &= ~(1<<UART_1_TX_PIN); UART_1_PORT_IN  |=  (1<<UART_1_TX_PIN); }   // TxD in input + pull-up
#       define Uart_tx_in_pu_off()  { UART_1_PORT_DIR &= ~(1<<UART_1_TX_PIN); UART_1_PORT_IN  &= ~(1<<UART_1_TX_PIN); }   // TxD in input no pull-up
#       define Uart_set_tx()        { UART_1_PORT_DIR |=  (1<<UART_1_RX_PIN); UART_1_PORT_OUT |=  (1<<UART_1_RX_PIN); }   // TxD in output + set pin
#       define Uart_clear_tx()      { UART_1_PORT_DIR |=  (1<<UART_1_RX_PIN); UART_1_PORT_OUT &= ~(1<<UART_1_RX_PIN); }   // TxD in output + clear pin
...
#       define Uart_rx_in_pu_on()       ((uart_selected==UART_0)? \
                                            (UART_0_PORT_DIR &= ~(1<<UART_0_RX_PIN), UART_0_PORT_IN  |=  (1<<UART_0_RX_PIN) ) \
                                            : \
                                            (UART_1_PORT_DIR &= ~(1<<UART_1_RX_PIN), UART_1_PORT_IN  |=  (1<<UART_1_RX_PIN) ) ) // RxD in input + pull-up
#       define Uart_rx_in_pu_off()      ((uart_selected==UART_0)? \
                                            (UART_0_PORT_DIR &= ~(1<<UART_0_RX_PIN), UART_0_PORT_IN  &= ~(1<<UART_0_RX_PIN) ) \
                                            : \
                                            (UART_1_PORT_DIR &= ~(1<<UART_1_RX_PIN), UART_1_PORT_IN  &= ~(1<<UART_1_RX_PIN) ) ) // RxD in input no pull-up
#       define Uart_rx_get_data()       ((uart_selected==UART_0)? \
                                            ((UART_0_PORT_IN  &   (1<<UART_0_RX_PIN)) >> UART_0_RX_PIN ) \
                                            : \
                                            ((UART_1_PORT_IN  &   (1<<UART_1_RX_PIN)) >> UART_1_RX_PIN ) )                      // Read RxD pin
#       define Uart_rx_get_3_data()     ((uart_selected==UART_0)? \
                                            ((UART_0_PORT_IN  & UART_0_PORT_IN  & UART_0_PORT_IN  & (1<<UART_0_RX_PIN)) >> UART_0_RX_PIN ) \
                                            : \
                                            ((UART_1_PORT_IN  & UART_1_PORT_IN  & UART_1_PORT_IN  & (1<<UART_1_RX_PIN)) >> UART_1_RX_PIN ) )    // Read 3 times RxD pin
#       define Uart_tx_in_pu_on()       ((uart_selected==UART_0)? \
                                            (UART_0_PORT_DIR &= ~(1<<UART_0_TX_PIN), UART_0_PORT_IN  |=  (1<<UART_0_TX_PIN) ) \
                                            : \
                                            (UART_1_PORT_DIR &= ~(1<<UART_1_TX_PIN), UART_1_PORT_IN  |=  (1<<UART_1_TX_PIN) ) ) // TxD in input + pull-up
#       define Uart_tx_in_pu_off()      ((uart_selected==UART_0)? \
                                            (UART_0_PORT_DIR &= ~(1<<UART_0_TX_PIN), UART_0_PORT_IN  &= ~(1<<UART_0_TX_PIN) ) \
                                            : \
                                            (UART_1_PORT_DIR &= ~(1<<UART_1_TX_PIN), UART_1_PORT_IN  &= ~(1<<UART_1_TX_PIN) ) ) // TxD in input no pull-up
#       define Uart_set_tx()            ((uart_selected==UART_0)? \
                                            (UART_0_PORT_DIR |=  (1<<UART_0_RX_PIN), UART_0_PORT_OUT |=  (1<<UART_0_RX_PIN) ) \
                                            : \
                                            (UART_1_PORT_DIR |=  (1<<UART_1_RX_PIN), UART_1_PORT_OUT |=  (1<<UART_1_RX_PIN) ) ) // TxD in output + set pin
#       define Uart_clear_tx()          ((uart_selected==UART_0)? \
                                            (UART_0_PORT_DIR |=  (1<<UART_0_RX_PIN), UART_0_PORT_OUT &= ~(1<<UART_0_RX_PIN) ) \
                                            : \
                                            (UART_1_PORT_DIR |=  (1<<UART_1_RX_PIN), UART_1_PORT_OUT &= ~(1<<UART_1_RX_PIN) ) ) // TxD in output + clear pin
#   else
...
#   else
#       define Uart_set_baudrate() ( Uart_set_ubrr(UART_BAUDRATE))
#endif
