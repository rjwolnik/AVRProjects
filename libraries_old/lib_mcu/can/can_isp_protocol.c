...
extern U8 nodeId;
extern U16 waitForBootMsg; // timer for countingdown 2 sec to get boot telegram
...
//--- Switch following CAN ISP resquest received
    ...
    //- SELECT MEMORY PAGE (0x06)    ---------------------------------
        case CAN_ID_SELECT_MEM_PAGE:
            ...
            //- Else nothing
            break;
    //- SELECT MEMORY PAGE (0x06)    --------------------------------- Added by WK !!!!
        case CAN_ID_SELECT_NODE_WK:
            //if (can_communication_opened)
            {
                if (can_isp_rx_buffer[0] == nodeId)
				{ 
				  PORTA&=~((1<<LED_RED)|(1<<LED_YELLOW)|(1<<LED_GREEN));
                  waitForBootMsg=0; 
				}
				else
				{
